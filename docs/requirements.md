...
**Travel times** between places live in a dedicated section near the bottom of this file (between `## Travel times (auto-generated — do not edit by hand)

_Last updated 2026-04-20 · OSRM public demo + Nominatim (OpenStreetMap) · driving car · 38 ok / 0 errors / 38 pairs._

_Claude invokes Nominatim + OSRM directly and writes this section. Build-time parser reads it verbatim and surfaces `→ 下一站` on each activity card._

| Pair ID | Route | Drive time |
|---|---|---|
| `haneda-airport` → `tokyo-station` | 羽田机场 → 东京站 | **28 min** / 19.9 km |
| `tokyo-station` → `karuizawa-station` | 东京站 → 轻井泽站 | **142 min** / 165.1 km |
| `kurumayama-skypark` → `karuizawa-kogen-church` | 车山高原 SKYPARK → 轻井泽高原教会 | **58 min** / 64.6 km |
| `karuizawa-kogen-church` → `karuizawa-station` | 轻井泽高原教会 → 轻井泽站 | **8 min** / 6.2 km |
| `karuizawa-station` → `tokyo-station` | 轻井泽站 → 东京站 | **145 min** / 165.7 km |
| `tokyo-station` → `shin-hakodate-hokuto-station` | 东京站 → 新函馆北斗站 | **711 min** / 851.4 km |
| `shin-hakodate-hokuto-station` → `hakodate-station` | 新函馆北斗站 → 函馆站 | **17 min** / 18.3 km |
| `toyako-silo-observation-deck` → `usuzan-ropeway` | 洞爷国立公园 筒仓展望台 → 有珠山缆车 · 洞爷湖展望台 | **22 min** / 16.7 km |
| `usuzan-ropeway` → `noboribetsu-jigokudani` | 有珠山缆车 · 洞爷湖展望台 → 登别 地狱谷 | **49 min** / 44.3 km |
| `sapporo-station` → `shirogane-blue-pond` | 札幌站 → 白金青池 | **132 min** / 151.3 km |
| `biei-christmas-tree` → `biei-seven-stars-tree` | 美瑛 圣诞树之木 → 美瑛 七星之木 | **5 min** / 4.4 km |
| `biei-seven-stars-tree` → `abashiri-city` | 美瑛 七星之木 → 网走市 | **185 min** / 216.4 km |
| `okhotsk-ryuhyo-museum` → `cape-notoro` | 鄂霍次克流冰馆 + 天都山展望台 → 能取岬 | **17 min** / 16.0 km |
| `cape-notoro` → `road-to-heaven-shari` | 能取岬 → 通天之路 | **51 min** / 63.4 km |
| `road-to-heaven-shari` → `oshinkoshin-falls` | 通天之路 → 奥辛科辛瀑布 | **21 min** / 27.4 km |
| `oshinkoshin-falls` → `masuura-viewpoint-parking` | 奥辛科辛瀑布 → 流冰街道 · 鳟浦 观景停车带 | **49 min** / 69.4 km |
| `cape-soya` → `noshappu-cape` | 宗谷岬 → 野寒布岬 | **28 min** / 35.4 km |
| `noshappu-cape` → `otaru-canal` | 野寒布岬 → 小樽运河 | **247 min** / 334.2 km |
| `rusutsu-resort` → `nakayama-toge` | 留寿都 度假村 → 中山峠 (岭) | **25 min** / 28.7 km |
| `nakayama-toge` → `otaru-canal` | 中山峠 (岭) → 小樽运河 | **55 min** / 62.3 km |
| `otaru-canal` → `otaru-music-box-museum` | 小樽运河 → 小樽 音乐盒堂 本馆 | **3 min** / 1.9 km |
| `otaru-music-box-museum` → `uomasa-otaru` | 小樽 音乐盒堂 本馆 → 鱼真 (小樽 寿司) | **864 min** / 1059.7 km |
| `uomasa-otaru` → `otaru-tengu-ropeway` | 鱼真 (小樽 寿司) → 小樽 天狗山 缆车 (山麓站) | **864 min** / 1059.2 km |
| `otaru-aquarium` → `sapporo-station` | 小樽水族馆 → 札幌站 | **39 min** / 41.8 km |
| `sapporo-station` → `hokkaido-university` | 札幌站 → 北海道大学 (札幌校区) | **1 min** / 0.3 km |
| `hokkaido-university` → `shiroi-koibito-park` | 北海道大学 (札幌校区) → 白色恋人公园 | **10 min** / 8.3 km |
| `nijo-market-sapporo` → `hill-of-the-buddha` | 二条市场 → 头大佛 (真驹内泷野灵园) | **1145 min** / 1414.8 km |
| `hill-of-the-buddha` → `okurayama-observatory` | 头大佛 (真驹内泷野灵园) → 大仓山展望台 (札幌冬奥跳台) | **1149 min** / 1417.1 km |
| `okurayama-observatory` → `kani-honke-sapporo` | 大仓山展望台 (札幌冬奥跳台) → 蟹本家 札幌驿前本店 | **8 min** / 5.7 km |
| `royce-cacao-chocolate-town` → `mt-moiwa-ropeway` | ROYCE' 可可与巧克力小镇 → 藻岩山 缆车 | **29 min** / 31.4 km |
| `mt-moiwa-ropeway` → `seafood-buffet-nanda` | 藻岩山 缆车 → 海鲜自助 NANDA | **6 min** / 4.5 km |
| `sapporo-station` → `shibuya-scramble` | 札幌站 → 涩谷 Scramble 十字路口 (住宿区附近) | **896 min** / 1086.7 km |
| `subaru-star-square-ebisu` → `tokyo-subaru-mitaka` | SUBARU 星广场（惠比寿旗舰展厅） → 东京 SUBARU 三鹰店 | **28 min** / 20.2 km |
| `tokyo-subaru-mitaka` → `shibuya-scramble` | 东京 SUBARU 三鹰店 → 涩谷 Scramble 十字路口 (住宿区附近) | **25 min** / 19.6 km |
| `shibuya-scramble` → `mominotakumi-shibuya` | 涩谷 Scramble 十字路口 (住宿区附近) → もみの匠 涩谷店 (按摩) | **1 min** / 0.3 km |
| `a-pit-autobacs-shinonome` → `nezu-museum` | A PIT 东雲 超级汽配 → 根津美术馆 | **12 min** / 11.2 km |
| `nezu-museum` → `gonpachi-nishiazabu` | 根津美术馆 → 权八 西麻布 (Kill Bill 原型居酒屋) | **2 min** / 1.1 km |
| `oedo-antique-market` → `star-tokyo-k-two-shibuya` | 大江户骨董市 → STAR TOKYO 涩谷店 (K-two 美发沙龙) | **8 min** / 7.9 km |

<!-- /travel-times -->
`). Claude populates it by directly querying OSM public services (Nominatim + OSRM). The Next.js build reads that section and surfaces drive times under each activity — ask Claude to refresh the table when the itinerary changes.
